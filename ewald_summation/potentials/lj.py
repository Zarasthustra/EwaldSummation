...
class LennardJones:
    ...
    def force(self, x):
        ...
        def f1(d):
            sigma6 = self.sigma ** 6
            gradient = -24 * self.epsilon * sigma6 * (sigma6 / d ** 14 - 1 / d ** 8)
            return gradient

        # force pairwise with switch_function
        def f2(d):
            sigma6 = self.sigma ** 6
            t = (x - self.cutoff) / (self.cutoff - self.switch_start)
            switch = 2 * t ** 3 + 3 * t ** 2
            potential = 4 * self.epsilon * sigma6 * (sigma6 / d ** 12 - 1 / d ** 6)
            dswitch = 6 / (self.cutoff - self.start) * (t ** 2 + t)
            gradient = 24 * self.epsilon * sigma6 * (sigma6 / d ** 14 - 1 / d ** 8)
            return -1 * (potential * dswitch + gradient * switch)
