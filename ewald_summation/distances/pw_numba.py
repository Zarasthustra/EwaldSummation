        ...
        if(self._pot_recalc):
            if(self.MULTI):
                self._pots = _pots_multi(self.n_particles, self.positions, self.l_box,
                                        self.n_image, self.image_grid, self.pot_func)
            else:
        ...
        if(self._force_recalc):
            if(self.MULTI):
                self._forces = _forces_multi(self.n_particles, self.positions, self.l_box,
                                        self.n_image, self.image_grid, self.force_func)
            else:
                self._forces = _forces_neighbor(self.n_cells, self.n_particles, self.neighbor_list_head,
                     self.neighboring_cells, self.neighbor_list, self.positions, self.l_box, self.force_func)
        ...
        while(particle_i != _None):
            for particle_j in neighboring_particles[:count]:
                if(particle_j != particle_i):
                    ...
                    pots_sum[particle_i] += pot_func((particle_i, particle_j), _calc_dist_neighbor(positions,
                                         particle_i, particle_j, l_box, half_l_box))
        ...
        while(particle_i != _None):
            for particle_j in neighboring_particles[:count]:
                if(particle_j != particle_i):
                    #yield(((particle_i, particle_j), _calc_dist_neighbor(positions,
                    #                     particle_i, particle_j, l_box, half_l_box)))
                    forces_sum[particle_i] += force_func((particle_i, particle_j), _calc_dist_neighbor(positions,
                                         particle_i, particle_j, l_box, half_l_box))
            particle_i = neighbor_list[particle_i, 1]
    return forces_sum
...
@njit
def _pots_multi(n_particles, positions, l_box, n_image, image_grid, pot_func):
    pots_sum = np.zeros(n_particles)
    for image in range(n_image):
        for i in range(n_particles):
            for j in range(n_particles):
                ...
                dist = np.sqrt(np.square(dv).sum())
                if(dist != 0.):
                    pots_sum[i] += pot_func((i, j), (dv, dist))
    return pots_sum

@njit
def _forces_multi(n_particles, positions, l_box, n_image, image_grid, force_func):
    n_dim = len(l_box)
    forces_sum = np.zeros((n_particles, n_dim))
    for image in range(n_image):
        for i in range(n_particles):
            for j in range(n_particles):
                ...
                dist = np.sqrt(np.square(dv).sum())
                if(dist != 0.):
                    forces_sum[i] += force_func((i, j), (dv, dist))
    return forces_sum
